...
async function loadDetailSales(Id, Detail) {
  ...
  try {
    ...
    document.getElementById("revision_number").value = `R${data.revision_number}` || `R0`;
    document.getElementById("status").value = data.status || "Draft";
    document.getElementById("catatan").value = data.catatan || "";
    document.getElementById("syarat_ketentuan").value = data.syarat_ketentuan || "";
    document.getElementById("term_pembayaran").value = data.term_pembayaran || "";
    ...
    if (!subcategoryRes.ok) {
      throw new Error(`Failed to load subcategories, status: ${subcategoryRes.status}`);
    }
    const subcatResponse = await subcategoryRes.json();
    const subcats = Array.isArray(subcatResponse.listData) ? subcatResponse.listData : [];
...
function calculateTotals() {
  ...
  document.querySelectorAll("#tabelItem .itemTotal, #tabelItem .subItemTotal").forEach(cell => {
...
async function saveInvoice(mode = "create", id = null) {
  try {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      const sub_category_id = parseInt(row.querySelector(".itemSubcategory")?.value || 0);
      ...
      const unit_price = parseRupiah(row.querySelector(".itemHarga")?.value || 0);
      ...
      if (subWrapper && subWrapper.classList.contains("subItemWrapper")) {
        ...
        subItems.forEach((sub) => {
          const subItemMaterial = sub.querySelector(".subItemMaterial")?.value.trim() || "";
          const subItemSpec = sub.querySelector(".subItemSpec")?.value.trim() || "";
          const subItemQty = parseInt(sub.querySelector(".subItemQty")?.value || 0);
          const subItemUnit = sub.querySelector(".subItemUnit")?.value.trim() || "pcs";
          const subItemHarga = parseRupiah(sub.querySelector(".subItemHarga")?.value || 0);
    ...
    if (res.ok) {
      Swal.fire("Sukses", `Quotation berhasil ${mode === "create" ? "dibuat" : "diupdate"}`, "success");
