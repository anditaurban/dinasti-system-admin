...
function renderTableHtml(data, isLocked) {
  ...
  if (data.length) {
    data.forEach((d) => {
      // 🔥 VIEW BUTTON TANPA VALIDASI FILE
      const viewBtn = `
      ...
      const actionButtons = `
        <div class="flex items-center justify-center">
          ${viewBtn}
          <button class="edit-btn text-blue-600 mr-2 hover:text-blue-800"
                  data-id="${d.payable_id}" type="button">✏️</button>
...
function handleViewProof(fileUrl) {
  ...
  // Karena di JSON field "file" sudah berisi full URL (https://...),
  // kita langsung pakai saja variablenya.
  Swal.fire({
    title: "Bukti Transaksi",
    imageUrl: fileUrl,
    imageAlt: "Bukti Transaksi",
    // Atur tinggi gambar agar responsif, atau gunakan width auto
    imageHeight: 400,
    showCloseButton: true,
    showConfirmButton: false, // Hilangkan tombol OK biar user fokus ke gambar/close X
  });
}
...
document.getElementById("purchaseBody").addEventListener("click", (e) => {
  ...
  if (e.target.closest(".view-proof-btn")) {
    ...
    const fileUrl = btn.dataset.file; // Ambil URL langsung dari data-file
